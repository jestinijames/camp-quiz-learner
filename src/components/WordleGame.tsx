...
export function WordleGame({ wordle, onComplete }: WordleGameProps) {
  ...
  // Check individual guess and get feedback from server
  ...
  const makeGuess = useCallback(async () => {
    ...
    // Check this guess with the server (THIS IS THE KEY FIX!)
    ...
    // Check if won (SERVER TELLS US THIS!)
    if (result.isCorrect) {
      setWon(true);
      setGameOver(true);
  ...
  }, [makeGuess, removeLetter, addLetter, submitting, gameOver]);
  ...
  return (
    <Card className="max-w-md mx-auto">
      <CardHeader>
        <CardTitle className="text-center text-lg font-bold break-words">
          {wordle.title}
        </CardTitle>
        <div className="text-center space-y-1">
          <p className="text-sm text-gray-600 break-words">{wordle.hint}</p>
      ...
      <CardContent className="space-y-4">
              ...
              <div key={rowIndex} className="flex justify-center gap-1">
                {rowIndex === 2 && (
                  <Button
                    ...
                    variant="outline"
                    className="px-2 py-2 text-xs font-medium h-10"
                  >
                    ENTER
                  </Button>
                )}
        ...
        {!gameOver && (
          <div className="text-center">
            <div className="text-lg font-bold tracking-wider text-blue-600 min-h-[24px]">
              ...
              <div className={`p-4 rounded-lg border-2 ${won ? "border-green-400 bg-green-50" : "border-orange-400 bg-orange-50"}`}>
                {won ? (
                  <div className="text-green-600">
                    ...
                    <p className="text-xl font-bold tracking-wider mt-2 text-green-800">{actualWord}</p>
                  </div>
                ) : (
